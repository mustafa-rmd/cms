...
public class AsyncImportController {
    ...
    public ResponseEntity<ImportJob> startAsyncImport(
        ...
        log.info("Async import request from provider '{}' by user '{}' from {} to {}",
                provider, userEmail, request.startDate(), request.endDate());
    ...
    @Operation(summary = "Get import job status", description = "Get the status and progress of an import job")
    ...
    @Operation(summary = "Get user's import jobs", description = "Get all import jobs for the current user")
    @GetMapping("/jobs/my")
    ...
    @Operation(summary = "Get import jobs by status", description = "Get import jobs filtered by status (ADMIN only)")
    @GetMapping("/jobs")
    public ResponseEntity<List<ImportJob>> getImportJobsByStatus(
            @Parameter(description = "Job status filter") @RequestParam(required = false) ImportJobStatus status) {
        
        authorizationService.requireUserManagement("view all import jobs");
        
        List<ImportJob> jobs = status != null 
    ...
    @Operation(summary = "Get import statistics", description = "Get import job statistics and metrics")
    @GetMapping("/stats")
    public ResponseEntity<Map<String, Object>> getImportStatistics() {
        ...
        stats.put("completedJobs", allJobs.stream().filter(j -> j.getStatus() == ImportJobStatus.COMPLETED).count());
        stats.put("failedJobs", allJobs.stream().filter(j -> j.getStatus() == ImportJobStatus.FAILED).count());
        stats.put("queuedJobs", allJobs.stream().filter(j -> j.getStatus() == ImportJobStatus.QUEUED).count());
    ...
    @Operation(summary = "Get available providers", description = "Get list of available external content providers")
